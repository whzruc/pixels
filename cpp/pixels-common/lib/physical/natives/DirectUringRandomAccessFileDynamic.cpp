...
std::shared_ptr<ByteBuffer> DirectUringRandomAccessFileDynamic::readAsync(int length, uint32_t colId) {
    ...
    // Get buffer from pool (uses current buffer set)
    ...
    // Get slot index for io_uring fixed buffer read
    // This returns the actual io_uring slot index for the current buffer set
