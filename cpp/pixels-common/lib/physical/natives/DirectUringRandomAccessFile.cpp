...
void DirectUringRandomAccessFile::RegisterBufferFromPool(std::vector <uint32_t> colIds)
{
    ...
    if (!isRegistered) {
        // auto buffersStart = std::chrono::high_resolution_clock::now();
        ...
        isRegistered = true;
        // auto buffersEnd = std::chrono::high_resolution_clock::now();
        // std::chrono::duration<double, std::milli> buffersDuration = buffersEnd - buffersStart;
        // std::cout << "DirectUringRandomAccessFile::RegisterBufferFromPool 耗时: "
        //           << buffersDuration.count() << " 毫秒" << std::endl;
...
bool DirectUringRandomAccessFile::RegisterMoreBuffer(int index, std::vector<std::shared_ptr<ByteBuffer>> buffers) {
    ...
    // 先检查缓冲区是否为空
    if (buffers.empty()) {
        throw InvalidArgumentException("DirectUringRandomAccessFile::RegisterMoreBuffer: 缓冲区列表为空");
    }

    struct iovec* iovecs = nullptr;
    iovecs = (iovec*)calloc(buffers.size(), sizeof(struct iovec));
    if (!iovecs) {
        throw InvalidArgumentException("DirectUringRandomAccessFile::RegisterMoreBuffer: 无法分配iovec内存");
    }
    ...
    for (auto i = 0; i < buffers.size(); i++)
    {
        ...
        if (!buffer) {
            free(iovecs);
            throw InvalidArgumentException("DirectUringRandomAccessFile::RegisterMoreBuffer: 缓冲区智能指针为空");
        }

        // 检查缓冲区大小是否合理
        if (buffer->size() <= 0) {
            free(iovecs);
            throw InvalidArgumentException("DirectUringRandomAccessFile::RegisterMoreBuffer: 缓冲区大小为0或负数");
        }
        ...
        if (!ptr) {
            free(iovecs);
            throw InvalidArgumentException("DirectUringRandomAccessFile::RegisterMoreBuffer: 缓冲区指针为空");
        }

        // 检查指针是否对齐（某些系统可能要求特定对齐方式）
        if (reinterpret_cast<uintptr_t>(ptr) % sizeof(void*) != 0) {
            std::cout << "警告: 缓冲区指针未按" << sizeof(void*) << "字节对齐，可能导致问题" << std::endl;
        }
        ...
        try {
            ...
            // 如果缓冲区可写，尝试写入再恢复（假设ByteBuffer是可写的）
            char temp = *static_cast<char*>(ptr);
            *static_cast<char*>(ptr) = temp; // 写入相同的值，测试可写性
        }
        catch (...) {
            free(iovecs);
            throw InvalidArgumentException("DirectUringRandomAccessFile::RegisterMoreBuffer: 缓冲区地址不可访问，地址=" +
        ...
        // 打印调试信息
        // std::cout << "缓冲区 " << i << " 地址: " << ptr
                  // << ", 大小: " << buffer->size() << " 字节" << std::endl;
    }
    ...
    if (ret != 0)
    {
        // 释放已分配的iovecs避免内存泄漏
        free(iovecs);
        // 从iovecs_vector中移除最后添加的元素
        if (!iovecs_vector.empty() && iovecs_vector.back() == iovecs) {
            iovecs_vector.pop_back();
        }

        // 构建具体错误信息
        std::string err_msg = "DirectUringRandomAccessFile::RegisterMoreBuffer: register buffer fails. ";
        switch (-ret) {
            case EINVAL:  err_msg += "无效参数(可能缓冲区数量/大小无效)"; break;
            case ENOMEM:  err_msg += "内存不足"; break;
            case ENOSPC:  err_msg += "缓冲区总数超过系统限制"; break;
            case EBUSY:   err_msg += "缓冲区已被注册，无法重复注册"; break;
            case EFAULT:  err_msg += "缓冲区地址无效(内核无法访问)"; break;
            case ENOSYS:  err_msg += "系统不支持该操作"; break;
            default:      err_msg += "未知错误(" + std::to_string(-ret) + ")";
        }
        throw InvalidArgumentException(err_msg);
    }
...
struct io_uring *DirectUringRandomAccessFile::getRing(int index) {
    if (index >= ring_vector.size()) {
        ...
        if (ring == nullptr) {
            ...
            if (ret < 0) {
                // 捕获错误并转换为具体描述
                std::string err_msg = "DirectRandomAccessFile: initialize io_uring fails. ";
                switch (-ret) {
                case ENOMEM:    err_msg += "内存分配失败"; break;
                case EINVAL:    err_msg += "无效参数(可能队列大小超限)"; break;
                case EMFILE:    err_msg += "进程描述符耗尽"; break;
                case ENFILE:    err_msg += "系统文件描述符耗尽"; break;
                case ENOSYS:    err_msg += "系统不支持io_uring(内核版本过低)"; break;
                case EPERM:     err_msg += "权限不足"; break;
                default:        err_msg += "未知错误(" + std::to_string(-ret) + ")";
                }
                delete ring; // 释放已分配的ring避免内存泄漏
                throw InvalidArgumentException(err_msg);
            }
        }
        ring_vector.emplace_back(ring);
