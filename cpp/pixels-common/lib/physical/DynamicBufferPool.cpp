...
std::shared_ptr<ByteBuffer> DynamicBufferPool::AllocateBuffer(uint32_t colId, uint64_t size) {
    ...
    // Allocate buffer from global pool (if initialized), otherwise use DirectIoLib
    std::shared_ptr<ByteBuffer> buffer;

    if (GlobalByteBufferPool::Instance().IsInitialized()) {
        buffer = GlobalByteBufferPool::Instance().AcquireBuffer(size);
    } else {
        buffer = directIoLib->allocateDirectBuffer(size);
        if (buffer == nullptr) {
            FreeSlot(slotIndex);
...
std::shared_ptr<ByteBuffer> DynamicBufferPool::GrowBuffer(uint32_t colId, uint64_t newSize) {
    ...
    // Allocate new larger buffer from global pool (if initialized), otherwise use DirectIoLib
    std::shared_ptr<ByteBuffer> newBuffer;
    if (GlobalByteBufferPool::Instance().IsInitialized()) {
        newBuffer = GlobalByteBufferPool::Instance().AcquireBuffer(newSize);
        // Copy old data to new buffer
        // memcpy(newBuffer->getPointer(), oldBuffer->getPointer(), oldBuffer->size());
        // Note: GlobalByteBufferPool already zeros the buffer, but we copy old data over
        
        // Return old buffer to global pool
        GlobalByteBufferPool::Instance().ReleaseBuffer(oldBuffer);
    } else {
        newBuffer = directIoLib->allocateDirectBuffer(newSize);
...
void DynamicBufferPool::ReleaseBuffer(uint32_t colId) {
    ...
    // Get the buffer before clearing the slot
    auto buffer = bufferSlots[currBufferIdx][slotIndex];
    
    // Return buffer to global pool (if initialized and buffer exists)
    if (buffer && GlobalByteBufferPool::Instance().IsInitialized()) {
        GlobalByteBufferPool::Instance().ReleaseBuffer(buffer);
    }
