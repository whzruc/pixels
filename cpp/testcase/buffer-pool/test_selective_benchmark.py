...
class ConfigurationTest(unittest.TestCase):
    ...
    def test_mode_properties_support_current_and_legacy_config_factory(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                (legacy_home / 'etc/pixels-cpp.properties').read_text(),
                explicit_path.read_text())
    ...
    def test_resume_artifact_requirements_include_perf(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name in ('perf-stat.csv', 'stacks.txt', 'stacks.folded',
                         'flamegraph.svg', 'symbolization.txt'):
                (case / name).write_text('ok')
            self.assertTrue(runner.required_artifacts_exist(case, 'legacy', 0, True))
            self.assertFalse((case / 'perf.data').exists())

    def test_resume_artifact_requirements_include_off_cpu(self):
        with tempfile.TemporaryDirectory() as directory:
            case = Path(directory)
            for name in ('resource.txt', 'timed.stdout', 'offcpu.folded',
                         'offcpu.svg', 'offcpu-symbolization.txt',
                         'offcpu-query.stdout'):
                (case / name).write_text('ok')
            for name in ('timed.stderr', 'offcpu.stderr', 'offcpu-query.stderr'):
                (case / name).touch()
            self.assertTrue(runner.required_artifacts_exist(
                case, 'legacy', 0, perf=False, off_cpu=True))
            (case / 'offcpu.folded').write_text('')
            self.assertFalse(runner.required_artifacts_exist(
                case, 'legacy', 0, perf=False, off_cpu=True))

    def test_perf_record_defaults_support_optimized_binaries(self):
        command = runner.perf_record_command(
            '/tmp/duckdb', Path('/tmp/perf.data'), 99, 'dwarf', 16384)
        self.assertIn('cpu-clock', command)
        self.assertIn('--call-graph=dwarf,16384', command)
        self.assertNotIn('--call-graph=fp', command)

    def test_off_cpu_duration(self):
        self.assertEqual(runner.off_cpu_trace_seconds('2.5', 0), 5)
        self.assertEqual(runner.off_cpu_trace_seconds('40.0', 0), 20)
        self.assertEqual(runner.off_cpu_trace_seconds(None, 0), 5)
        self.assertEqual(runner.off_cpu_trace_seconds('40.0', 7), 7)

    def test_symbolization_report(self):
        with tempfile.TemporaryDirectory() as directory:
            folded = Path(directory) / 'stacks.folded'
            report = Path(directory) / 'symbolization.txt'
            folded.write_text('duckdb;known 75\nduckdb;[unknown];leaf 20\n[unknown] 5\n')
            self.assertEqual(runner.write_symbolization_report(folded, report), 25.0)
            text = report.read_text()
            self.assertIn('unknown_stack_pct=25.000000', text)
            self.assertIn('unknown_leaf_pct=5.000000', text)
